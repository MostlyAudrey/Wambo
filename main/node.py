...
class Node:
    ...
    def set(self, list, x, y, goal=None):
        # type: ([Node], float, float, (int, int, int)) -> None
        self.neighbors = list
        self.x = x
        self.y = y
        self.goal = goal
    ...
    def checkCollision(self, x, y):
        ...
        if self.x - self.node_size / 2 < x < self.x + self.node_size / 2:
            if self.y - self.node_size / 2 < y < self.y + self.node_size / 2:
                return True
        return False
